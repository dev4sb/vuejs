...
async function availableFilters(context, params, customQuery) {
  ...
  const filters = context.extendQuery(customQuery, {
    filters: {
      query: DEFAULT_QUERY$1,
      variables
    }
  });
  const response = await context.client.apolloClient.query({
    query: gql(filters.query),
...
function convertFacetFiltersLocalToShopify(filters) {
  ...
  for (const key of Object.keys(filters)) {
    if (PRICE_FILTER_RANGE.includes(key)) {
      result.price = result.price ? __spreadProps(__spreadValues({}, result.price), { [key]: parseFloat(filters[key]) }) : { [key]: parseFloat(filters[key]) };
    }
  }
  return result;
}
async function getCollection(context, params, customQuery) {
  var _a2;
  const variables = {
